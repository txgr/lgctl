...
service {{.apiServiceName}} {
    ...
    put /{{.modelNameSnake}}/update ({{.modelName}}Info) returns (BaseMsgResp)

    // Delete {{.modelEnglishName}} information | 删除{{.modelChineseName}}信息
    @handler delete{{.modelName}}
    delete /{{.modelNameSnake}}/delete ({{if .useUUID}}UU{{end}}IDs{{.IdType}}Req) returns (BaseMsgResp)

    // Get {{.modelEnglishName}} list | 获取{{.modelChineseName}}信息列表
    @handler get{{.modelName}}List
    get /{{.modelNameSnake}}/list ({{.modelName}}ListReq) returns ({{.modelName}}ListResp)

    // Get {{.modelEnglishName}} by ID | 通过ID获取{{.modelChineseName}}信息
    @handler get{{.modelName}}ById
    get /{{.modelNameSnake}} ({{if .useUUID}}UU{{end}}ID{{.IdType}}FormReq) returns ({{.modelName}}InfoResp)
}
