...
import (
	...
	"github.com/txgr/lgctl/util/ctx"
	"github.com/txgr/lgctl/util/entx"
	"github.com/txgr/lgctl/util/format"
	"github.com/txgr/lgctl/util/pathx"
	"github.com/txgr/lgctl/util/protox"
)
...
func GenLogicByProto(p *GenLogicByProtoContext) error {
	...
	for _, v := range models {
		...
		if genCtx.GenApiData {
			...
			_, err := execx.Run(fmt.Sprintf("lgctl extra init_code -m %s -t other -n %s%s", genCtx.ModelName, genCtx.RPCServiceName, prefixStr), genCtx.OutputDir)
			if err != nil {
